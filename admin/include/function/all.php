...
function combo_database_v3($tabel, $id_field, $field, $query) {
    if ($query == '') {
        $sql = mysql_query("SELECT * FROM $tabel");
    } else {
        $sql = mysql_query($query);
    ...
    if (mysql_num_rows($sql) != 0) {
		$state=[];
        while ($data = mysql_fetch_assoc($sql)) {
			if(!in_array($data[$id_field],$state)){
				
                ?>
                <option value="<?php echo $data[$id_field]; ?>">
                    <?php 
					echo baca_database("","jenis_latihan","select * from data_jenis_latihan where id_jenis_latihan='{$data[$id_field]}'") ;
					?>
                </option>
